...
const DataModal = ({ name, path, event, title, onEdit, onCreate, children }) => {
	...
	const onSubmit = async (newData) => {
		...
		if (isEmpty(errors)) {
			if (isEditMode) {
				if (data !== newData) {
					isFunction(onEdit) ? await onEdit(newData) : dispatch({
						type: "on_edit_item",
						payload: {
							path,
							value: newData
				...
				isFunction(onCreate) ? await onCreate(newData) : dispatch({
					type: "on_add_item",
					payload: {
						path,
						value: newData
	...
	const getTitle = isEmpty(title) ? getModalText(isEditMode, name, t)
