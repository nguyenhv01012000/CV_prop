...
const ContactItem = ({ value, icon, link }) => {
  ...
  return value ? (
    <div className="flex items-center" style={{ color: data.metadata.colors.background }}>
      <Icon
        size="10px"
        className="mr-2"
      ...
      {link ? (
        <a href={link} target="_blank" rel="noopener noreferrer" style={{ color: data.metadata.colors.background }}
        >
          <span className="font-medium break-all">{value}</span>
        </a>
      ) : (
          <span className="font-medium break-all" style={{ color: data.metadata.colors.background }}
          >{value}</span>
        )}
    </div>
  ) : null;
