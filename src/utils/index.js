...
export const dataSynchronization = (state, sections) => {
	for (const section of sections) {
		const heading = localStorage.getItem(section);
		if (heading == null) localStorage.setItem(section, state[section]['heading'])
		else state[section]['heading'] = heading;
	}
	...
	if (primaryColor == null) localStorage.setItem('primaryColor', state['metadata']['colors']['primary']);
	else state['metadata']['colors']['primary'] = primaryColor;
	if (textColor == null) localStorage.setItem('textColor', state['metadata']['colors']['text']);
	else state['metadata']['colors']['text'] = textColor;
	if (backgroundColor == null) localStorage.setItem('backgroundColor', state['metadata']['colors']['background']);
	else state['metadata']['colors']['background'] = backgroundColor;
	if (template == null) localStorage.setItem('template', state['metadata']['template']);
	else state['metadata']['template'] = template;
	if (fontSize == null) localStorage.setItem('fontSize', state['metadata']['fontSize']);
	else state['metadata']['fontSize'] = fontSize;
	if (cvName == null) localStorage.setItem('cvName', state['metadata']['name']);
	else state['metadata']['name'] = cvName;
	return state;
}
